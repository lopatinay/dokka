...
def save_file(file, upload_uuid):
    ...
    reverse_task = Task(
        status=TaskStatus.pending, task_type=TaskType.reverse, upload_uuid=upload_uuid
    )
    distance_task = Task(
        status=TaskStatus.pending, task_type=TaskType.distance, upload_uuid=upload_uuid
    )
